...
">
    <div class="flex items-center justify-between mb-6">
        ...
        <template x-if="!syncing">
            <form method="POST" action="{{ route('invoices.sync') }}">
                @csrf
                <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-[#4A90D9] text-white hover:bg-[#3A7BC8] transition-all duration-200 cursor-pointer btn-shadow">
    ...
    @if($invoices->count() > 0 || !empty(array_filter($filters ?? [])))
        <div class="bg-white dark:bg-[#161615] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-4 mb-4 card-accent">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                        ...
                        <input id="filter-search" type="text" name="search" value="{{ $filters['search'] ?? '' }}"
                               ...
                               class="w-full px-3 py-2 rounded-lg border border-[#e3e3e0] dark:border-[#3E3E3A] bg-white dark:bg-[#161615] text-sm focus:ring-2 focus:ring-[#4A90D9]/30 focus:border-[#4A90D9] outline-none transition">
                    </div>

                    <div>
                        <label for="filter-status" class="block text-xs text-[#706f6c] dark:text-[#A1A09A] mb-1">{{ __('Status') }}</label>
                        <select id="filter-status" name="status" class="w-full px-3 py-2 h-[38px] rounded-lg border border-[#e3e3e0] dark:border-[#3E3E3A] bg-white dark:bg-[#161615] text-sm focus:ring-2 focus:ring-[#4A90D9]/30 focus:border-[#4A90D9] outline-none transition">
                    ...
                    <div>
                        ...
                        <select id="filter-type" name="type" class="w-full px-3 py-2 h-[38px] rounded-lg border border-[#e3e3e0] dark:border-[#3E3E3A] bg-white dark:bg-[#161615] text-sm focus:ring-2 focus:ring-[#4A90D9]/30 focus:border-[#4A90D9] outline-none transition">
                            <option value="">{{ __('All') }}</option>
        ...
        @if(!empty(array_filter($filters ?? [])))
            <div class="flex flex-wrap gap-2 mb-4">
                @if(($filters['status'] ?? ''))
                    <span class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-[#EBEBE8] dark:bg-[#2a2a28] text-[#5a5a57] dark:text-[#9a9a95]">
    ...
    @if($invoices->count() > 0)
        <div class="bg-white dark:bg-[#161615] rounded-lg shadow-md border border-[#e3e3e0] dark:border-[#3E3E3A] overflow-hidden">
                            ...
                            <td class="px-6 py-4">
                                @if(in_array($invoice->status, [\App\Enums\InvoiceSyncStatus::Pending, \App\Enums\InvoiceSyncStatus::Failed]))
                                    <form method="POST" action="{{ route('invoices.retry-sync', $invoice) }}" class="inline">
                                        @csrf
                                        <button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-[#4A90D9] text-white hover:bg-[#3A7BC8] transition-all duration-200 cursor-pointer btn-shadow">
    ...
    @else
        <div class="bg-white dark:bg-[#161615] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-12 text-center">
            ...
            <p class="text-[#706f6c] dark:text-[#A1A09A] font-medium">{{ __('No invoices yet.') }}</p>
