...
">
    <div class="flex items-center justify-between mb-6">
        ...
        <template x-if="!syncing">
            <form @submit="submitted = true" method="POST" action="{{ route('invoices.sync') }}">
                @csrf
            ...
            <form method="GET" action="{{ route('invoices') }}" class="space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    @if(!empty(array_filter($filters ?? [])))
                        <a href="{{ route('invoices') }}" class="inline-flex items-center gap-1.5 text-sm text-accent hover:underline">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        <label for="filter-type" class="block text-xs text-ink-muted mb-1">{{ __('Type') }}</label>
                        <select id="filter-type" name="type" class="w-full px-3 py-2 h-[38px] rounded-lg border border-line bg-surface-1 text-sm focus:ring-2 focus:ring-accent-ring/30 focus:border-accent-ring outline-none transition">
                            <option value="">{{ __('All') }}</option>
                    ...
                    <div class="flex gap-2">
                        <div class="flex-1 min-w-0">
                            <label for="filter-amount-from" class="block text-xs text-ink-muted mb-1">{{ __('From') }}</label>
                            <input id="filter-amount-from" type="number" name="amount_from" value="{{ $filters['amount_from'] ?? '' }}"
                                   step="0.01" min="0" placeholder="0"
                                   class="w-full px-3 py-2 rounded-lg border border-line bg-surface-1 text-sm focus:ring-2 focus:ring-accent-ring/30 focus:border-accent-ring outline-none transition">
                        </div>
                        <div class="flex-1 min-w-0">
                            <label for="filter-amount-to" class="block text-xs text-ink-muted mb-1">{{ __('To') }}</label>
            ...
            <div class="flex flex-wrap gap-2 mb-4">
                ...
                @if(($filters['type'] ?? ''))
                    <span class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-tone-neutral-soft text-tone-neutral-label">
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['type' => null]) }}" class="hover:text-ink">&times;</a>
                    </span>
                @endif
                @if(($filters['amount_from'] ?? ''))
                    <span class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-tone-neutral-soft text-tone-neutral-label">
                        {{ __('Amount') }}: {{ $filters['amount_from'] }} - {{ $filters['amount_to'] ?? str('&infin;')->toHtmlString() }}
                        <a href="{{ request()->fullUrlWithQuery(['amount_from' => null, 'amount_to' => null]) }}" class="hover:text-ink">&times;</a>
                    </span>
                @endif
                ...
                <tbody class="divide-y divide-line">
                    @foreach($invoices as $invoice)
                        <tr class="hover:bg-surface-2 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-ink-muted" title="{{$invoice->created_at->toDateTimeString()}}">{{ $invoice->created_at->diffForHumans() }}</td>
                            <td class="px-6 py-4">
                                @if(in_array($invoice->status, [\App\Enums\InvoiceSyncStatus::Pending, \App\Enums\InvoiceSyncStatus::Failed]))
                                    <form x-data="{ loading: false }" @submit="loading = true" method="POST" action="{{ route('invoices.retry-sync', $invoice) }}" class="inline">
                                        @csrf
                                        <button type="submit" :disabled="loading" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-accent text-on-accent hover:bg-accent-hover transition-all duration-200 cursor-pointer btn-shadow disabled:opacity-60 disabled:cursor-not-allowed">
                                            ...
                                            <span x-text="loading ? '{{ __('Retrying…') }}' : '{{ __('Retry Sync') }}'">{{ __('Retry Sync') }}</span>
                                        </button>
                                    </form>
                                @endif
                            </td>
    ...
    @else
        <div class="bg-surface-1 rounded-lg shadow-sm border border-line p-12 text-center">
            ...
            <p class="text-ink-muted font-medium">{{ __('No invoices yet.') }}</p>
            <p class="text-sm text-ink-muted mt-1">{{ __('Sync your Foodics orders to see them here.') }}</p>
        </div>
    @endif
</div>
@endsection
