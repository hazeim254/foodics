...
This application is an integration middleware between Foodics and Daftra applications.
...
- For each change in frontend code, please make sure to run `npm run build` to ensure the frontend is up to date.
...
- Every user-facing string added to a view or message (UI text, validation messages, flash messages, etc.) must be
  wrapped for translation and given an Arabic translation.
- Use the full English string itself as the translation key — no prefixes, suffixes, or dot-namespacing. Example:
  `__('Save changes')`, not `__('save_changes')` or `__('messages.save')`.
- Add the Arabic entry to the Arabic translation file (`lang/ar.json`). Do not create or maintain an English translation
  file — Laravel falls back to the key, which is already the English copy.
